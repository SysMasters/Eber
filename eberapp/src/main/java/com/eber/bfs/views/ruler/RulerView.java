package com.eber.bfs.views.ruler;
...
public class RulerView extends View {
    ...
    public RulerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        interval = array.getDimensionPixelSize(R.styleable.RulerView_interval, dp2px(intervalsBetweenValues));
        fromValue = array.getInt(R.styleable.RulerView_fromValue, 0);
        toValue = array.getInt(R.styleable.RulerView_toValue, intervalsBetweenValues);
        currentValue = array.getInt(R.styleable.RulerView_currentValue, (fromValue + toValue) / 2);
        intervalsBetweenValues = array.getInt(R.styleable.RulerView_intervalsBetweenValues, intervalsBetweenValues);
        valuesInterval = array.getInt(R.styleable.RulerView_valuesInterval, 1);
        valuesTextSize = array.getDimensionPixelSize(R.styleable.RulerView_valuesTextSize, sp2px(16));
        valuesTextColor = array.getColor(R.styleable.RulerView_valuesTextColor, Color.BLACK);
        linesWidth = array.getDimensionPixelSize(R.styleable.RulerView_linesWidth, dp2px(1));
        linesColor = array.getColor(R.styleable.RulerView_linesColor, Color.BLACK);
        orientation = array.getInt(R.styleable.RulerView_orientation, HORIZONTAL);

        array.recycle();
        selectLineColor = Color.parseColor("#03a9f4");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (orientation == VERTICAL) {
            ...
            while (true) {
                ...
                position--;
                if (position < fromValue / valuesInterval * intervalsBetweenValues)
                    break;
                height -= interval;
                if (height < 0 - textHeight)
                    break;
            }
            ...
            while (true) {
                position++;
                if (position > toValue / valuesInterval * intervalsBetweenValues)
                    break;
                height += interval;
                if (height > getHeight() + textHeight)
                    break;
                if (position % (intervalsBetweenValues / 2) == 0) {
                    if (position % intervalsBetweenValues == 0) {
                        canvas.drawLine(getWidth(), height, getWidth() / 2, height, paint);
            ...
            while (true) {
                ...
                if (width < 0 - paint.measureText("10000"))
                    break;
            }
            ...
            while (true) {
                ...
                if (position > toValue / valuesInterval * intervalsBetweenValues)
                    break;
                width += interval;
                if (width > getWidth() + paint.measureText("10000"))
                    break;
                if (intervalsBetweenValues == 1) {
                    String valueString = Integer.toString(position / intervalsBetweenValues * valuesInterval);
                    paint.setColor(valuesTextColor);
